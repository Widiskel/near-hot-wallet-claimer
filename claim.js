...
const AccountCall = (accountId, privateKey) => new Promise(async (resolve, reject) => {
    try {
...
const miningProgress = (detailUser) => {
    ...
    const timeSinceLastClaimHours = (currentTime - (detailUser.last_claim / 1e6)) / (1000 * 60 * 60);
...
const getHotPerHourInt = (detailUser) => {
    ...
    return new BigNumber((woodBooster.value * fireplaceBooster.value)).dividedBy(1e7);
};
